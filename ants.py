...
# Name: Arjun Mehta 
# Email: arjunmehta94@berkeley.edu
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant():
            ...
            if self.ant != None:
                ...
                elif inect.can_contain(self.ant):
...
class Ant(Insect):
    ...
    def can_contain(self, other_ant):
        if self.ant.container == True and self.ant == None and other_ant.container == False:
            return True
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        i = 0
        while current_place.name != hive.name :
            if (current_place.bees != [] and i>= self.min_range and i<=self.max_range):
...
class FireAnt(Ant):
    ...
    implemented = False
    food_cost = 4
    #bees_copy = list(Place(name).bees)
    def reduce_armor(self, amount):
        "*** YOUR CODE HERE ***"
        """
        if self.place:
            for item in bees_copy:
                if self.bees.place and self.armor<=0:
                    Insect.reduce_armor(item, damage)
        implemented = True
        """
        place = self.place
        Insect.reduce_armor(self, amount)
        if self.armor<= 0:
            bees_copy = list(place.bees)
...
class NinjaAnt(Ant):
    """NinjaAnt is an Ant which does not block the path and does 1 damage  
    to
...
class QueenPlace:
    ...
    def __init__(self, colony_queen, ant_queen):
        ...
        self.impostor_check = False

    @property
    def bees(self):
        "*** YOUR CODE HERE ***"
        if self.colony_queen:
